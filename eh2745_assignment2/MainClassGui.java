...
public class MainClassGui {
	...
	private void initialize() {
		try {
			...
			double[][] points = new double[200][18];

			// save the values in one row
			double[] pointsrow = new double[18];
			for (int j1 = 0; j1 < 200; j1++) {
				for (int j2 = 0; j2 < 9; j2++) {
					points[j1][j2] = voltList.get(j1 * 9 + j2).getvalue();
					points[j1][j2 + 9] = angList.get(j1 * 9 + j2).getvalue();
				}
			}
			for (int z = 0; z < 200; z++) {
				for (int y = 0; y < 18; y++) {
			...
			// inserting the values in a new matrix
			double[][] pointstest = new double[200][18];

			// save the values in one row
			double[] pointsrowtest = new double[18];
			for (int j1 = 0; j1 < 20; j1++) {
				for (int j2 = 0; j2 < 9; j2++) {
					pointstest[j1][j2] = voltListtest.get(j1 * 9 + j2)
							.getvalue();
					pointstest[j1][j2 + 9] = angListtest.get(j1 * 9 + j2)
							.getvalue();
				}
			}

			// set initial 4 points
			double[] initialmean1 = new double[18];
			double[] initialmean2 = new double[18];
			double[] initialmean3 = new double[18];
			double[] initialmean4 = new double[18];
			...
			double[] newcenter1 = new double[18];
			double[] newcenter2 = new double[18];
			double[] newcenter3 = new double[18];
			double[] newcenter4 = new double[18];
			for (int m = 0; m < 18; m++) {
			...
			//
			double[] value1 = new double[18];
			double[] value2 = new double[18];
			double[] value3 = new double[18];
			double[] value4 = new double[18];
			...
			for (int integrator = 0; integrator < 10; integrator++) {
				for (int t = 0; t < 200; t++) {
					double[] pointvalue = new double[18];
			...
			for (int x = 0; x < cluster2.size(); x++) {
				...
				pointslearning[cluster1.size() + x][18] = 2;
			}
			;
			...
			for (int x = 0; x < cluster3.size(); x++) {
				...
				pointslearning[cluster1.size() + cluster2.size() + x][18] = 3;
			}
			;
			...
			// cluster set, 20*200
			double[] pointvaluetest = new double[18];
			double[][] distance = new double[20][200];
			for (int x = 0; x < 20; x++) {
				for (int t = 0; t < cluster1.size(); t++) {
					for (int y = 0; y < 18; y++) {
			...
			btnPerformKnnAlgorithm.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if (resultpopup == JOptionPane.OK_OPTION) {
						...
						Object[][] outputprint = new Object[20][1];
						// for cluster1
						for (int t = 0; t < 20; t++) {
							...
							double c3 = 0;
							double c4 = 0;
							double[] pointvalue = new double[200];
